...
# ===================== GET VISUALIZATIONS =====================s
# convert country name to ISO codes
...
def get_choropleth_map():
    ...
    # create the map and set customizations
    ...
    # Set the layout and look of the map
    fig.update_layout(
...
# Function to get the donut chart for top AI tools used in a country
def get_donut_chart(df_country):
    # get the percentages and labels
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            itemwidth=30,          # make legend items narrower
            itemsizing="constant", # important for consistent item width
        ),
...
# Function to get the bar chart for AI-generated content distribution by industry
def get_content_bar_chart(df_country):
    # Group by industry and sum the AI-generated content volume
    df_industry = (
    ...
    # Calculate the percentage of AI-generated content volume per industry
    ...
    # adjust the style of the traces, x-axis, and y-axis of the bar char
    fig.update_traces(
        marker = dict(
            color="#6d7883",
            line=dict(
...
# Function to get the heatmap of correlations between industry metrics
def get_heatmap(df_country):   
    ...
    # Short labels for the heatmap to get a better overview
    short_labels = [
    ...
    # create hovertexts => they need to stay in the long format for clarity
    hover_text = [
    ...
    # configure layout and style of the heatmap and axes
    fig.update_layout(
        title=dict(
...
# Function to get the radar chart for average AI metrics per industry
def get_radar_chart(df_country):
    # get the unique industries from the dataframe and define the relevant metrics
    industries = df_country["Industry"].unique()
    ...
    fig = go.Figure() # create the figure for the radar chart

    # calculate the average values for each industry and add the traces to the radar chart
    ...
    # Set the layout and style of the radar chart
    fig.update_layout(
        polar=dict(
            bgcolor="#2c2c2c",
            radialaxis=dict(
...
# Function to get the line chart for AI impact metrics over the years
def get_line_chart(df_country):
    fig = go.Figure()

    # group by the year and calculate the mean for each metric
    ...
    # short labels for the metrics in the legend to get a better overview
    short_metrics = [
    ...
    # adjust the layout and style of the line chart and axes
    fig.update_layout(
        title=dict(
        ...
        yaxis=dict(
            title=dict(
                text="Percentage (%)",
                font=dict(size=9),
                standoff=5  # distance from the axis line
            ),
            range=[0, 103],  # set range to 0-100% with a little margin
            tickfont=dict(size=9),
            rangemode="tozero",
            title_font=dict(size=9),
            showgrid=True,            # show grid lines
            gridcolor="rgba(255,255,255,0.35)",  # subtle lines for the grid
            tickmode="array",
            tickvals=[0, 25, 50, 75, 100],
            gridwidth=0.5             # thickness of the grid lines
        ),
        legend=dict(
            orientation="h",
            yanchor="top",
            y=-0.10,           # closer to the bottom
            xanchor="center",
            x=0.5,
            font=dict(size=9),
            itemwidth=40,      # width of each legend item
            itemsizing="constant",
            valign="top",
            traceorder="normal"
        ),
...
# convert strictness value to label
...
# convert label to strictness value
...
# Function to get the gauge chart for average regulation strictness
def get_gauge(df_country):
    ...
    # add the average strictness label at the bottom of the gauge
    fig.add_annotation(
    ...
    # adjust the layout and style of the gauge chart
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",  # transparent background
        plot_bgcolor="rgba(0,0,0,0)",  # transparent plot background
        font=dict(color=TEXT_COLOR, size=10),
...
# Function to get the total AI content generated in TBs
def get_total_ai_volume(df_country):
    # calculate the total AI-generated content volume in TBs
    total_ai_content_tb = df_country["AI-Generated Content Volume (TBs per year)"].sum()

    # create the figure with the total AI content in TBs
    fig = go.Figure(go.Indicator(
        mode="number",
    ...
    # adjust the layout and style of the total AI content chart
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",  # transparent background
        plot_bgcolor="rgba(0,0,0,0)",  # transparent plot background
        font=dict(color=TEXT_COLOR, size=10),
...
# Function to get the bubble chart for AI metrics => mainly set dropdowns and slider options
def get_bubble_chart(df):
    # create a slider for the years in the bubble chart and add the "All" option to the slider
    ...
    return html.Div([
        ...
        html.Div([
            # dropdown for the options for the x-axis of the bubble chart
            html.Div([
            ...
            # slider for the year which is displayed int the bubble chart
            html.Div([
                ...
                dcc.Slider(
                    ...
                    tooltip=None,
                    included=True,
                    updatemode='mouseup',  # update on mouse release
                )
...
# Function to get the big line chart for AI metrics over the years => mainly set dropdowns and filter options
def get_big_line_chart():
    return html.Div([
        # explorative line chart
        html.Div([
        ...
        html.Div([
            # dropdown for the filter options
            html.Div([
            ...
            # dropdown for the group options
            html.Div([
            ...
            # dropdown for the metric selection
            html.Div([
        ...
        # filter options when a filter is selected
        html.Div([
                # dropdown for selecting which values to include based on the applied filter
                dcc.Dropdown(
                    id='metric-dropdown',
                    options=[],
                    value=None,
                ...
                # button to select or deselect all metrics
                html.Button(
                    "All", 
...
# ===================== CALLBACKS FOR INTERACTING WITH THE DASHBOARD =====================
...
def render_content(selected_country):
    if selected_country is None:
        # if no country is selected, show the choropleth map
        return html.Div(id="map-area", children=[
            ...
            dcc.Graph(id="choropleth-map", figure=choropleth_map, style={"height": "70vh", "margin": "5px 40px 0 40px"})
        ])
    else:
        # if a country is selected, show the detailed analysis
        ...
        return html.Div([
            ...
            # create a grid layout for the analysis charts and place the visualizations in the grid
            html.Div([
                html.Div([
                    # donut chart
                    html.Div(
                    ...
                    # gauge and total AI content
                    html.Div([
                        # Gauge chart at the top
                        html.Div(
                            dcc.Graph(figure=get_gauge(df_country), config={"displayModeBar": False},
                                    style={"width": "100%", "height": "100%"}),
                            style={"flex": "1", "backgroundColor": BACKGROUND_COLOR, "color": TEXT_COLOR}
                        ),

                        # horizontal line below the gauge
                        html.Div(style={"height": "10px", "backgroundColor": BASE_COLOR}),

                        # Indicator for total AI content at the bottom
                        html.Div(
...
# callback to update the choropleth map based on the selected country
@callback(
...
def update_bubble_chart(x, year):    
    ...
    # if the year is set to "All", calculate the mean for each country and industry and the regulation status
    ...
    # create the figure for the bubble chart and set basic properties
    fig = px.scatter(
        filtered_df,
        x=x,
        y='AI Adoption Rate (%)',
    ...
    # set the hover template for the bubble chart
    fig.update_traces(
    ...
    # set the layout and style of the bubble chart and axes
    fig.update_layout(
        title=dict(
...
# callback to update the donut chart based on the selected country
@callback(
...
def update_big_line_chart(selected_group, filter_value, filtered_selected_metrics, select_metric):    
    ...
    # select the metrics to be displayed in the line chart
    all_metrics = [
        "Consumer Trust in AI (%)",
        "Revenue Increase Due to AI (%)",
        "Job Loss Due to AI (%)",
    ...
    if filter_value is not None:
        df_filtered = df[df[filter_value].isin(filtered_selected_metrics)]
    
     # show the AI-Generated Content Volume over the years
    if selected_group == "Metrics":
        # group by year and calculate mean for all metrics
        select_metric = None # set to None, otherwise y-axis could have a wrong format
    ...
    # show the AI-Generated Content Volume over the years
        ...
        # group by year and calculate the sum of AI-Generated Content Volume
    ...
    # show the selected metric over the years grouped by the selected group
    else:
        # group by the selected group and calculate the mean or sum for the selected metric
        ...
        # add traces for each group value
    ...
    # set the title based on the selected group and metric
    ...
    # set the maximum value for the y-axis based on the selected group and metric
    ...
    # update the layout and style of the line chart and axes
    fig.update_layout(
        title=dict(
            text=title_text,
            x=0.5,
...
# callback to update the filter options based on the selected group
@callback(
...
# callback to update the metric dropdown options based on the selected group => not all filter options are available for all groups
@callback(
...
def update_applied_filter_line_chart(selected_group):
    # set the options and default values based on the selected group
    if selected_group == 'Metrics':
        options = [
...
# callback to toggle the visibility of the filter dropdown based on whether a filter is selected
@callback(
...
# callback to toggle the visibility of the select metric container based on the selected group
@callback(
...
def toggle_select_metric_container_visibility(selected_group):
    if selected_group != 'Metrics' and selected_group != 'Generated Content Volume':
        return {"width": "33%", "paddingRight": "10px"}, {"width": "33%", "paddingRight": "10px"}, {"width": "33%"} # layout for 3 dropdowns
    else:
        return {"width": "50%", "paddingRight": "10px"}, {"width": "50%"}, {'display': 'none'} # layout for 2 dropdowns => hide select metric container
    

# callback to select or deselect all metrics in the filter dropdown
@callback(
...
choropleth_map = get_choropleth_map() # get the choropleth map so it has not to be created again in the callback all the time
...
# set the basic layout of the dash app
app.layout = html.Div([
    dcc.Store(id='selected-country', data=None), # store the selected country for the analysis
